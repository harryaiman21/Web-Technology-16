...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen bg-background flex">
      <Navbar />
      <main className="flex-1 max-w-6xl mx-auto px-4 py-8 overflow-auto h-screen w-full">
        {/* Recently processed banner */}
        {recentlyProcessed.length > 0 && (
          <div className="bg-green-500/10 border border-green-500/30 rounded-none p-4 mb-6 flex items-center gap-3">
            ...
            {[
              ...
              <Card key={s.label} className="text-center">
                <CardContent className="pt-4 pb-3">
                  <div
                    className={`text-2xl font-black text-white ${s.color} rounded-none py-1 mb-1`}
                  >
                    {s.value}
                  </div>
            ...
            <div className="flex flex-wrap gap-2">
              {allTags.map((t) => (
                <Badge
                  ...
                  className={`cursor-pointer transition-all ${tag === t ? "bg-red-600 text-white" : "hover:bg-red-500/10 hover:text-red-200"}`}
